...
;                   (km).  3-element double array [x, y, z].
...
;   DS_TARGET - Nominal spacing between integration points in km.
...
;   H_CONRATH - Passed through to osse_conrath_density.  Default: 10.0d0 km.
;
; OUTPUTS:
...
function osse_calculate_transmittance, sat_pos, sun_dir, intersections, n_int, $
  ...
  if n_elements(ds_target) eq 0 then ds_target = 1.0d0
